...
def image_detection(image_path, network, class_names, class_colors, thresh):
    ...
    t1 = time.time()
    print('read image time', t1 - t0)
    print('image shape', image.shape)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    image_resized = cv2.resize(image_rgb, (width, height),
                               interpolation=cv2.INTER_LINEAR)
    t2 = time.time()
    print('resize image time', t2 - t1)
    print('image_resized shape', image_resized.shape)

    darknet.copy_image_from_bytes(darknet_image, image_resized.tobytes())
    t3 = time.time()
    print('copy image time', t3 - t2)

    detections = darknet.detect_image(network, class_names, darknet_image, thresh=thresh)
    t4 = time.time()
    print('detect time', t4 - t3)
    darknet.free_image(darknet_image)
    t5 = time.time()
    print('free image time', t5 - t4)
...
def main():
    ...
    while True:
        ...
        print("detection image time:", time.time() - prev_time)
