...
from utils import get_pos_2d, get_pos_3d, get_rectangle_region_depth
...
class GLViewer:
    ...
    def get_bounding_box(self, _bbox:list, pc:sl.Mat, param_matrix:np.matrix)->list:
        max_dist  = 100000000.0
        _x, _y, _w, _h = _bbox
        region_depth = get_rectangle_region_depth(pc, int(_x), int(_y), 5, 5)
        if -max_dist < region_depth < 0:
            front_depth = region_depth
            back_depth = region_depth - 0.1

            max_height = pc.get_height()

            left_pixel = np.mat([_x - _w/2., _y, 1]).transpose()
            right_pixel = np.mat([_x + _w/2., _y, 1]).transpose()
            # bottom_pixel = np.mat([_x, max_height - _y - _h/2., 1]).transpose()
            # top_pixel = np.mat([_x, max_height - _y + _h/2., 1]).transpose()
            bottom_pixel = np.mat([_x, _y - _h/2., 1]).transpose()
            top_pixel = np.mat([_x, _y + _h/2., 1]).transpose()
            
            left_coord = param_matrix * left_pixel
            right_coord = param_matrix * right_pixel
            bottom_coord = param_matrix * bottom_pixel
            top_coord = param_matrix * top_pixel
            
            region_dist = np.fabs(region_depth)

            # print('left_coord', left_coord)
            # print('right_coord', right_coord)
            # print('bottom_coord', bottom_coord)
            # print('top_coord', top_coord)

            # left_value = np.array(left_coord)[0][0] * region_dist
            # right_value = np.array(right_coord)[0][0] * region_dist
            # bottom_value = np.array(bottom_coord)[1][0] * region_dist
            # top_value = np.array(top_coord)[1][0] * region_dist

            # left_value = np.array(left_coord)[0][0] * region_depth
            # right_value = np.array(right_coord)[0][0] * region_depth
            # bottom_value = np.array(bottom_coord)[1][0] * region_depth
            # top_value = np.array(top_coord)[1][0] * region_depth
            ...
            # print('left_value, right_value, bottom_value, top_value', left_value, right_value, bottom_value, top_value)
            # print('bbox', _bbox)
            # print('region_depth', region_depth)
            # print('real width', right_value - left_value)
            # print('real height', top_value - bottom_value)

            left_value, right_value = right_value, left_value
            bottom_value, top_value = top_value, bottom_value

            pt_list = [(left_value, top_value, front_depth), (left_value, top_value, back_depth), 
                                (right_value, top_value, back_depth), (right_value, top_value, front_depth),
                                (left_value, bottom_value, front_depth), (left_value, bottom_value, back_depth),
                                (right_value, bottom_value, back_depth), (right_value, bottom_value, front_depth)]
            return pt_list
        else:
            return []

    def update_scene_view(self, _image:sl.Mat, _point_cloud:sl.Mat, detection_list:list, scale_2d:np.ndarray, param_matrix:np.matrix):
        self.mutex.acquire()
        ...
        for label, confidence, bbox in detection_list:
            ...
            x, y, w, h = scaled_bbox
            # print("{}: {}%\t bbox".format(label, confidence), scaled_bbox)

            bounding_pt_list = self.get_bounding_box(scaled_bbox, _point_cloud, param_matrix)
            # print('bounding_pt_list', bounding_pt_list)

            if len(bounding_pt_list) > 0:
                color_id = generate_color_id(obj_index)
                pos_3d = get_pos_3d(_point_cloud, x, y)
                self.create_id_rendering(pos_3d, color_id, obj_index)

                self.create_bbox_rendering(bounding_pt_list, color_id)
